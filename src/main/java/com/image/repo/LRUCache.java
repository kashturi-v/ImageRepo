...
/**
 * LRUCache hold the mainly functionality for our doubly linked priority queue,
 * where each priority node hold another doubly linked queue of images. It has
 * instances of the queue's head and tail, and a hashmap of the image's that
 * come into the repo.
 */
public class LRUCache {
    ...
    /**
     * @desc isEmptyPQ is used to determine whether the PQ is empty
     * @return (boolean) - returns whether it is empty or not
     */
    ...
    /**
     * @param imageName (String) - name of the new image imageLocation (String) -
     *                  location of the new image priority (int) - priority of the
     *                  current user
    ...
    public void enterPQ(String imageName, String imageLocation, int priority) {
        ...
        if (this.imageRepo.containsKey(imageName)) {
            System.out
                    .println("An image with the same name already exists, please choose try again with another name.");
            return;
        }
    ...
    public ImageNode leavePQ(int priority) {
        if (isEmptyPQ()) {
            ...
            System.out.println(
                    "Unfortunately, the current user does not have the appropriate permissions to delete any of the saved images.");
            return null;
        }
    }
    ...
    public void deleteImageNode(String imageName, int priority) {
        ...
        if (imgNode.priority < priority) {
            System.out.println(
                    "Unforutnately, the current user does not have the appropriate permissions to delete image \""
                            + imageName + "\".");
            return;
        }
        ...
        if (imgNode.prev == null) {
            ...
            while (startPQNode != null) {
                ...
                if (queue.head == imgNode) {
                    queue.popHeadQ();
                    // so in the case where its the last node in the queue, we have to delete the
                    // queue from the whole PQ
                    if (queue.head == null) {
     ...
     * @desc getImageNode gets the ImageNode corresponding to the image name
     * @return (ImageNode) - returns the image node
     */
